...
for m in measure_list['measure_name']:
    ...
    if len(measure_description) > 0:
        ...
        data["percentile"] = data.groupby(["month"])["rate"].rank(pct=True).to_numpy()
...
# -
# %%capture
# !jupyter nbconvert --to html --execute SICBL_improvement_radar.ipynb --no-input --ExecutePreprocessor.timeout=-1
